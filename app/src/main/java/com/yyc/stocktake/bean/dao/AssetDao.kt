...
interface AssetDao {
     ...
     * 根据label跟订单id 查询
     */
    @Query("SELECT * FROM AssetBean WHERE asset_labelTag =:LabelTag AND asset_orderNo = :orderId AND asset_roNo = :userId AND asset_companyId = :companyId AND asset_inventoryStatus != 2")
